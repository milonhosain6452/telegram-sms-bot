...
def add_watermarks(image: Image.Image) -> Image.Image:
    """
    Add two banner-style watermarks on the image:
    1. Fb page = Glimxoo  (left middle)
    2. search = avc + search & join all channel (bottom)
    """
    draw = ImageDraw.Draw(image)
    try:
        font = ImageFont.truetype("arial.ttf", 40)  # বড় সাইজ ফন্ট
    except:
        font = ImageFont.load_default()

    W, H = image.size

    # 1. Fb Page (Middle Left)
    text1 = "Fb page = Glimxoo"
    tw1, th1 = draw.textsize(text1, font=font)
    x1, y1 = 20, (H - th1) // 2
    draw.rectangle([x1 - 10, y1 - 10, x1 + tw1 + 10, y1 + th1 + 10], fill=(0, 0, 0, 128))
    draw.text((x1, y1), text1, font=font, fill=(255, 255, 255))

    # 2. Telegram Search (Bottom)
    text2 = "search = avc"
    text3 = "search & join all channel"
    tw2, th2 = draw.textsize(text2, font=font)
    tw3, th3 = draw.textsize(text3, font=font)

    total_h = th2 + th3 + 20
    x2, y2 = (W - tw2) // 2, H - total_h - 20
    x3, y3 = (W - tw3) // 2, H - th3 - 20

    max_w = max(tw2, tw3)
    draw.rectangle([min(x2, x3) - 10, y2 - 10, min(x2, x3) + max_w + 20, y3 + th3 + 10], fill=(0, 0, 0, 128))

    draw.text((x2, y2), text2, font=font, fill=(255, 255, 255))
    draw.text((x3, y3), text3, font=font, fill=(255, 255, 255))

    return image

def apply_blur_to_image_bytes(image_bytes: bytes, radius: float) -> bytes:
    """
    Apply Gaussian blur + watermark to image bytes and return resulting image bytes (JPEG).
    """
